...
def MakeFastConversionMovie(image_eigenvectors,big_image_matrix,moment_matrix,truth_matrix,pkl_name):
    ...
    for img in range(0,len(big_image_matrix)):
        ...
        list_evt_weight += [1./image_angle_err]
    ...
    print (f'arrival model = {np.around(model,3)}')
    ...
    target = list_impact
    model, chi = linear_regression(list_latent_space, target, list_evt_weight)
    print (f'impact model = {np.around(model,3)}')
    print (f'pkl_name = {pkl_name}')
    for img in range(0,len(target)):
        predict = linear_model(list_latent_space[img], model)
        list_predict += [predict]
    ...
    model, chi = linear_regression(list_latent_space, target, list_evt_weight)
    print (f'log_energy model = {np.around(model,3)}')
    print (f'pkl_name = {pkl_name}')
    for img in range(0,len(target)):
        predict = linear_model(list_latent_space[img], model)
        list_predict += [predict]
...
def MakeFastConversionImage(image_eigenvectors,big_image_matrix,time_eigenvectors,big_time_matrix,moment_matrix,truth_matrix,pkl_name):
...
def MakeLookupTable(eigenvectors,big_matrix,moment_matrix,truth_matrix,image_rank,pkl_name,nvar=3):
    ...
    for img in range(0,len(big_matrix)):
        ...
        for r in range(0,image_rank):
            lookup_table[r].fill(arrival,impact,log_energy,weight=image_latent_space[r]*1./image_angle_err)

        lookup_table_norm.fill(arrival,impact,log_energy,weight=1./image_angle_err)
